...
type HydraReader(reader: Npgsql.NpgsqlDataReader) =
    ...
    member __.``public.account`` =
        ``public``.Readers.accountReader (reader, buildGetOrdinal typeof<``public``.account>)

    member __.``public.favorite`` =
        ``public``.Readers.favoriteReader (reader, buildGetOrdinal typeof<``public``.favorite>)

    member __.``public.schemaversions`` =
        ``public``.Readers.schemaversionsReader (reader, buildGetOrdinal typeof<``public``.schemaversions>)

    member __.``public.talk`` =
        ``public``.Readers.talkReader (reader, buildGetOrdinal typeof<``public``.talk>)
